...
- [x] Setup Maven + Java 17 (shade JAR)
- [x] Grid I/O + validación (rectangularidad, símbolos, bordes `#`, coherencia de `+`, rechazo de cruces con `.`)
- [x] Modelo base (`Grid`, `CellType`, `Direction`, `TrafficLight`)
- [x] Ocupación `occ[]` (cell*4) + restricción de no mezclar ejes
- [x] Inicialización determinista con `--seed`
- [x] Motor secuencial con timestamps + export `ticks.csv`
- [x] Motor paralelo por agentes (thread pool) con determinismo (CAS min) + timestamps + export `ticks.csv`
- [x] BenchmarkRunner con warm-up + repeticiones + `summary.csv` (speedup/efficiency)
- [ ] Tests JUnit 5 (parser, semáforos, movimiento, integración seq vs par)
- [ ] Reporte técnico (5–8 páginas) con gráficas y conclusiones
